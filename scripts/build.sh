...
# 检查环境变量（支持 GITHUB_TOKEN 或 VERCEL_TOKEN）
TOKEN="${GITHUB_TOKEN:-$VERCEL_TOKEN}"

if [ -z "$TOKEN" ]; then
    echo "❌ 错误: 未设置 GITHUB_TOKEN 或 VERCEL_TOKEN 环境变量"
    echo ""
    echo "配置步骤："
    ...
    echo "   名称: GITHUB_TOKEN 或 VERCEL_TOKEN"
    echo "   值: <你的 GitHub token>"
    exit 1
fi

echo "✅ 检测到 GitHub token 环境变量"
